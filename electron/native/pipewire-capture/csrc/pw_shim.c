                          ...
                          struct osc_pw_frame *out)
{
    ...
    if (is_dmabuf_import) {
        ...
        if (chunk_size == 0 || height <= 0 ||
            SPA_FLAG_IS_SET(chunk_flags, SPA_CHUNK_FLAG_CORRUPTED)) {
            return 0;
        }
        /*
        ...
        bounds_error = osc_resolve_frame_bounds(data->type, data->maxsize, mapped_len,
                                                chunk_offset, chunk_size, chunk_flags, stride,
                                                width, height, &available, &offset, &size);
